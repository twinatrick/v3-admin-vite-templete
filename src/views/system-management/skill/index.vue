...
const onRowDbClick = (row: SkillVo) => {
  editDialogRef.value?.show(row)
}

const onRowSelect = (row: SkillVo) => {
  selectedRow.value = row
}
</script>

<template>
  <div class="app-container" id="skill-page">
    <!-- 查詢表單 -->
    <QueryForm
      ref="queryFormRef"
    ...
    >
      ...
      <QueryFormItem label="創建者" :col-span="1">
        <el-input v-model="skillListFilters.createdBy" placeholder=" " clearable />
      </QueryFormItem>
    </QueryForm>

...
export const getVisitedView = () => {
  const views = getVisitedViews()
  const route = useRoute()
  return views.length ? views.find((item) => item.name === route.name)?.meta?.queryData : {}
}
...
//#region 通知
export const getSysMsgFilter = () => {
  const json = localStorage.getItem(CacheKey.SYSMSG_FILTER)
  return JSON.parse(json ?? "[]") as string[]
}
export const setSysMsgFilter = (filterList: string[]) => {
  localStorage.setItem(CacheKey.SYSMSG_FILTER, JSON.stringify(filterList))
}
//#endregion

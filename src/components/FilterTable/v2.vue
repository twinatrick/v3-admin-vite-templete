...
import CustomTable from "@/components/CustomTable/v2.vue"
...
import { PropType, computed, reactive, ref, useAttrs, useSlots, watch } from "vue"
...
<template>
  ...
  >
    ...
    <template #default>
      ...
      <template v-for="tableColumn in prop.tableColumnMap" :key="tableColumn.name">
        ...
        >
          <template #header>
            ...
            <MultipleSelectWithSelectAll v-if="tableColumn.type === 'select'" v-model="searchItem[tableColumn.name]">
      ...
      >
        <template #header>
          <el-input-with-tag v-model="searchItem[extraMapColumn.name]" placeholder="Fuzzy Search" allow-empty />
        </template>
      </CustomTableColumn>
      <el-table-column fixed="right" :width="extraWidth" :label="extraLabel" v-if="extraColumn">
        <template #default="scope">
